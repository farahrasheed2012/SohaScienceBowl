...
struct DrillSubjectBarRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(name).font(GameFont.caption()).foregroundStyle(GameColors.textPrimary)
                Spacer()
                Text("\(correct) / \(total)")
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(spacing: 12) {
                        ForEach(Array(subjectRows.enumerated()), id: \.offset) { index, row in
                            DrillSubjectBarRow(name: row.name, correct: row.correct, total: row.total, color: row.color)
                    ...
                    if let onSecondary, let secondaryActionTitle {
...
struct ConfettiView: UIViewRepresentable {
    func makeUIView(context: Context) -> UIView {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) { emitter.birthRate = 0 }
        return view
    }
    ...
    private func makeCell() -> CAEmitterCell {
        ...
        cell.spin = 3
        cell.scale = 0.06
        cell.color = [UIColor(GameColors.biology), UIColor(GameColors.chemistry), UIColor(GameColors.physics), .systemYellow].randomElement()?.cgColor
...
struct DrillFlashOverlay: View {
    ...
    var body: some View {
        if let color {
            color.opacity(0.18).ignoresSafeArea().allowsHitTesting(false).transition(.opacity)
        }
    }
}
